...
public class AlchemistCraftingContainer extends RecipeBookContainer<CraftingInventory>{
	private final CraftingInventory craftMatrix = new CraftingInventory(this, 3, 3);
	private final CraftResultInventory craftResult = new CraftResultInventory();
	private final IWorldPosCallable canInteractWithCallable;
	private final PlayerEntity player;
	
	public AlchemistCraftingContainer(int windowId, final PlayerInventory playerInventory, final PacketBuffer data) {
		this(windowId, playerInventory, IWorldPosCallable.NULL);
	}
	
	public AlchemistCraftingContainer(int windowId, final PlayerInventory playerInventory, IWorldPosCallable worldPosCallable) {
		super(ModContainerTypes.ALCHEMIST_CRAFTING.get(), windowId);
		this.canInteractWithCallable = worldPosCallable;
		this.player = playerInventory.player;
	    this.addSlot(new AlchemyCraftingResultSlot(playerInventory.player, this.craftMatrix, this.craftResult, 0, 124, 35));
	...
	protected static void slotChangedCraftingGrid(int p_217066_0_, World world, PlayerEntity p_217066_2_, CraftingInventory craftInv, CraftResultInventory p_217066_4_) {
		if (!world.isClientSide) {
			...
			Optional<IAlchemyRecipe> optional = world.getServer().getRecipeManager().getRecipeFor(RecipeSerializerInit.ALCHEMY_TYPE, craftInv, world);
			if (optional.isPresent()) {
				...
				IAlchemyRecipe ialchemyrecipe = optional.get();
				if (p_217066_4_.setRecipeUsed(world, serverplayerentity, ialchemyrecipe) && ialchemyrecipe.isUnlocked(iProfession.getSkillTree())) {
					itemstack = ialchemyrecipe.assemble(craftInv);
				}
			}
	...
	public boolean stillValid(PlayerEntity playerIn) {
		return stillValid(this.canInteractWithCallable, playerIn, BlockInit.ALCHEMISTCRAFTINGTABLE_BLOCK.get());
	}
